...
\thispagestyle{empty}
	...
	This, in fact, holds for any field theory\footnote{
		Reference: discussions in \textit{Polchinski}, \S8.2. 
	...
	Note that here we are working in the grand canonical ensemble, where we have temperature $\beta = 2\pi\tau_2$ and chemical potential $2\pi\tau_1$. At this stage $P$ is \textit{not} fixed, and we should sum over all states with various $P$. To go to the canonical ensemble, we do a Legendre transform and trade $\tau_1$ for some $P'$. This is implemented by a Laplace / Fourier transform of the partition function:
	\begin{equation}
		\int \dd{\tau_1} e^{-i\,(2\pi\tau_1) P'}
	\end{equation}
	In string theory, we should actually work with the canonical ensemble (and eventually, the \textit{micro-}canonical ensemble), since we would like to impose the \textit{level matching condition}, namely $P' = 0$. This is because we would like to gauge the worldsheet translation along $\sigma^1$ generated by $P$, which is understood as a redundancy. 
	
	However, in string theory, the Fourier transform is implicit as we integrate along $\tau_1$ in the moduli space; roughly speaking, we have:
	\begin{equation}
	...
	As $\tau_1$ gets integrated out, we are effectively computing $\Tr' e^{-\beta H}$ where the trace only goes over the $P = 0$ sector of the Hilbert space\footnote{
		This is nicely explained in David Tong's \href{https://www.damtp.cam.ac.uk/user/tong/string.html}{\textit{String Theory}}. See also \S6.3 of Blumenhagen et al, \textit{Basic Concepts of String Theory}. See also \S2.1, 2.2 of \arxiv{1912.07654} where the switch between ensembles is reviewed in the context of holography. 
	}. 
	...
	For the second term, note that the action of $r$ on a state can be deduced from its action on the modes $\alpha_m, \tilde{\alpha}_m$, which is in turn induced from the action $r\colon X\mapsto -X$ through the mode expansion \eqref{eq:modes_circle}. In particular, we have $r: \alpha_{-m} \mapsto -\alpha_{-m}$, which means that\footnote{
		Reference: Blumenhagen et al, \textit{Basic Concepts of String Theory}, \S10.5. 
	}:
	\begin{equation}
		r\circ\alpha_{-m}\circ r^{-1}
		= -\alpha_{-m},
	\quad
		r \ket{N_m}
		\sim r \circ \alpha_{-m}^{N_m} \ket{0}
		\sim (-1)^{N_m} \ket{N_m},
	\end{equation}
	\begin{equation}
		r\colon\ %
			...
			\longmapsto (-1)^{\sum_l (N_l + \tilde{N}_l)}
				\ket{(N_l),(\tilde{N}_l);-n,-w}
	\end{equation}
	In particular, it reverses $n,w$, hence $r$ insertion gives vanishing amplitude unless $n = w = 0$. 
	The summation is very much similar to the $Z_{S^1}$ case, i.e. we have:
	\begin{equation}
	\begin{aligned}
	...
	Physically, $r$ invariant states are stationary waves:
	\begin{equation}
		\frac{1 + r}{2}
			\ket*{(N_l),(\tilde{N}_l);n,w}
		= \frac{1}{2} \pqty{
			\ket*{(N_l),(\tilde{N}_l);n,w}
			+ (-1)^{\sum_l (N_l + \tilde{N}_l)}
			\ket*{(N_l),(\tilde{N}_l);-n,-w}
		},
	\end{equation}
	which is a superposition of states with opposite momentum and winding, such that $\ave{p_L} = \ave{p_R} = 0$. Note that a general $r$ invariant state is a linear combination of such states and may have non-vanishing momentum: $\ave{p} \ne 0$. 
	In position space these states have unconstrained $x = x_L + x_R \in [0,\pi R]$. More precisely, if we define the position basis: $x\ket{x'} = \delta(x - x') \ket{x'}$, then we have:
	\begin{equation}
	\begin{aligned}
		\mel{(N_l),(\tilde{N}_l);x}{\frac{1 + r}{2}}{
			(N_l),(\tilde{N}_l);n
		}
		&\propto \frac{1}{2} \pqty{
			e^{i\frac{n}{R}x}
			+ (-1)^{\sum_l (N_l + \tilde{N}_l)}
			e^{-i\frac{n}{R}x}
		} \\
		...
		\begin{aligned}
			& \cos\tfrac{nx}{R}, 
			&&\textstyle\sum_l (N_l + \tilde{N}_l)\,\ \text{even}
		\\[.5ex]
			& \sin\tfrac{nx}{R}, 
			&&\textstyle\sum_l (N_l + \tilde{N}_l)\,\ \text{odd}
		\end{aligned}
	\end{aligned}
	\end{equation}
	
	The wave function $(\cos\frac{nx}{R})$ is what we expect: we get the same result if we turn off the oscillator modes $N, \tilde{N}$ altogether; in this case we have a worldline quantum mechanics with target $S^1/\mbb{Z}_2$, which is equivalent to the spacetime QFT on $S^1/\mbb{Z}_2$. Since we are only considering the worldline geometry $S^1$, we are restricting to the 1-particle sector of the spacetime QFT, which is again a quantum mechanical system. The wave functions compatible with $\mbb{Z}_2$ action are precisely given by $(\cos\frac{nx}{R})$. 
	On the other hand, the case with $(\sin\tfrac{nx}{R})$ doesn't seem to be $\mbb{Z}_2$ invariant in the position space. This is due to the $\mbb{Z}_2$ odd stringy excitations, which can be understood as some additional degrees of freedom, similar to the spin of a particle. This compensates for the $\mbb{Z}_2$ odd $(\sin\tfrac{nx}{R})$, and the full wave function is $\mbb{Z}_2$ invariant. 
	...
	\end{gather}
	Apply the boundary condition on $X$, and we find that $x = \pi R w$; however, due to the identification $X + 2\pi R \cong X \cong -X$, there are only two inequivalent choices:
	\begin{equation}
		x = 0,\quad x =\pi R,
	\end{equation}
	which correspond to the string localized around either of the two fixed points of the $\mbb{Z}_2$ action. There is no momentum or winding, not even in the mode expansion. 
	
	Intuitively, the twisted boundary condition is realized only when the string is stretched around the fixed points of the orbifold action. For example, an open string that ends on $X$ and $X + 2\pi R \in \mbb{R}^1$ becomes a closed winding string when we consider $\mbb{R}^1/(2\pi R\,\mbb{Z}) = S^1$. Similarly, an open string that ends on $\pm X$ becomes a closed string in the twisted sector, after we take $S^1/\mbb{Z}_2$. 
	...
	\begin{equation}
		...
		= \frac{1}{2}\,Z_{S^1}(\tau)
			...
			+ \abs{\frac{\eta(\tau)}{\theta_3(\tau)}}
	\end{equation}
	The first term is modular invariant, as is proven in \boxed{1}\,. 
