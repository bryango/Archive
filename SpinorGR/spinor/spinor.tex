...
\pagebreak[4]
	...
	Generally, We define \textit{spin group} $\mrm{Spin}(p,q)$ as the $\mbb{Z}_2$ extension (double cover) of $\mrm{SO}(p,q)$\footnote{
		...
		For example, $\pi_1\pqty\big{\mrm{Spin}(2,1)} = \mbb{Z}$ infinite cyclic, this gives rise to fractional spin $s\in\mbb{Q}_+$ (\textit{anyons}) in $(2+1)$ dimensions \cite{Wilczek:1982wy}. If we consider central extension of the Lie algebra, then the particle spin can even take arbitrary $s\in\mbb{R}_+$ values. 
