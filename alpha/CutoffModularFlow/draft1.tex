...
\subsection{Boundary modular flows}
	...
	\begin{equation}
		...
		s_{+1} = \frac{1}{u_+ - u_-}
	\end{equation}
	Where the $H_i$'s are given by $L_n,\tilde{L}_n$ with $n=0,\pm 1$, i.e.~the global conformal generators, and $(u,v)_\pm$ or $(t,x)_\pm$ marks the endpoints of the interval $\mcal{A}$. The overall coefficient $2\pi$ is fixed by requiring that $\kappa_\xi = 2\pi$. 
	...
	\begin{equation}
		x_\pm = \pm \frac{L_\mrm{CFT}}{2} \cosh \theta,
	\quad
		t_\pm = \pm \frac{L_\mrm{CFT}}{2} \sinh \theta,
	\end{equation}
	...
	\begin{gather}
	\begin{aligned}
		\xi_\mrm{CFT} &= \frac{2\pi}{L_\mrm{CFT}} \bigg\{
			\pqty\Big{
				\pqty{
					(\tfrac{L_\mrm{CFT}}{2})^2
					- (t^2 + x^2)
	...
	\begin{equation}
		\xi_\mrm{CFT}|_{t = 0} 
		= \frac{2\pi}{L_\mrm{CFT}}
			\pqty{
				\pqty{\frac{L_\mrm{CFT}}{2}}^2
				- x^2
			} \,\pdd{t}
	\end{equation}
	
	However, after the $T\bar{T}$-deformation, the scaling and special conformal transformations are no longer symmetries of the theory, so we cannot simply take $\xi_\mrm{CFT}$ as the modular flow in the deformed theory. 
	...
	\item The dependence of $\xi_{T\bar{T}}|_{t = 0}$ w.r.t.~$L$ is very similar to the dependence of $\xi_\mrm{CFT}|_{t = 0}$ w.r.t.~$L_\mrm{CFT}$, except that now the normalization is slightly modified to include the deformation parameter $\lambda$. 
	\item This is not yet a proposal for the full modular flow, but only its values around the $t = 0$ slice. But we shall see that this is enough to give us the entanglement entropy of a $T\bar{T}$-deformed theory. 
	\end{enumerate}
...
\subsection{Effective local temperature and entanglement entropy}
	...
	\end{equation}
	\begin{equation}
	S= \frac{c}{3} \int_{-L_\mrm{CFT}/2}^{L_\mrm{CFT}/2}
			\frac{L_\mrm{CFT}/2}{(L_\mrm{CFT}/2)^2-x^2}dx,
	\end{equation}
	The indefinite integral $
		\int \frac{L_\mrm{CFT}/2}{(L_\mrm{CFT}/2)^2-x^2}dx
		= \mop{arctanh}\frac{2x}{L_\mrm{CFT}}
%		\rightarrow\mrm{CFT}
	$ diverges as $x\rightarrow L_\mrm{CFT}/2$, so one needs a cutoff:
	\begin{equation}
	S=\frac{c}{3}\int_{-L_\mrm{CFT}/2+\epsilon}^{L_\mrm{CFT}/2-\epsilon}
		\frac{L_\mrm{CFT}/2}{(L_\mrm{CFT}/2)^2-x^2}dx
	=\frac{2c}{3} \mop{arctanh} \pqty{
			1-\frac{2\epsilon}{L_\mrm{CFT}}
		}
	\end{equation}
	...
	\begin{equation}
		\mop{arctanh} \pqty{
			1-\frac{2\epsilon}{L_\mrm{CFT}}
		}\rightarrow \frac{1}{2}\log\frac{L_\mrm{CFT}}{\epsilon}
	\end{equation}
	So we get the usual result
	\begin{equation}
	S=\frac{c}{3}\log\frac{L_\mrm{CFT}}{\epsilon}
	\end{equation}
	
	Now for the cutoff field theory living at the boundary, where
	\begin{equation}
...
%			} \log Z[\beta]
	...
	***
	...
	\end{equation}
	The relation between $l_b$ and $l$ comes from the gravitational side,
	\begin{equation}
...
\subsection{Rindler map in finite temperature}
	...
	\begin{equation}
	z=\frac{1}{ir_1}e^{ir_1w},\ \ \ r_1=\frac{2\pi}{\beta}
	\end{equation} 
	...
	e^{ir_1(w+\bar{w})}
	\end{equation}
	Note that compared to the usual map from cylinder to plane $z=e^{ir_1 w}$, we put the overall scaling factor $\frac{1}{ir_1}$ to make sure that the conformal factor at the endpoints is $1$. From 1806.07444, the entanglement entropy for $l_p$ is
	\begin{equation}
...
%	\quad
	...
	Again, we restrict to an interval centered at the origin: $
		...
		= (\frac{L_\infty}{2})^2
	$, where $x_\pm = -x_\mp$, and same for $t_\pm, u_\pm, v_\pm$. 
	$L_\infty$ is the coordinate length of the boundary interval $\mcal{A}$ at the asymptotic infinity; this corresponds to the field theory side $L_\mrm{CFT}$. 
