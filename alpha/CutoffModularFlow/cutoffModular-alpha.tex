...
\abstract{
	...
	\item We then note that the periodicity can be restored by following the flow of $\xi$ starting from a codim-2 cutoff surface. This could be related to the \textit{restricted maximin} proposal of \cite{Marolf:2019bgj,Grado-White:2020wlb}. 
	
	\item We try to obtain the entanglement entropy using the modular flow at the \mbox{codim-2} cutoff surface, by defining a \textit{local entanglement entropy $\beta(x)$}, which relates the entanglement entropy to some thermal entropy. 
...
\textbf{Summary:} We try to obtain the entanglement entropy using the modular flow at the \mbox{codim-2} cutoff surface, by defining a \textit{local entanglement entropy $\beta(x)$}, which relates the entanglement entropy to some thermal entropy. 
	...
	We note that there is a trick to read out the entanglement entropy from the modular flow: it is possible to define a \textit{local entanglement temperature}, and then use it to read the entanglement entropy from the modular flow generator, by comparing with the Hamiltonian of a thermal field theory, where we have:
	\begin{equation}
	...
	Let's see this shortcut in action in CFT$_2$; the \textit{entropy density} is given by:
	\begin{equation}
	\partial_x S = \frac{c}{3} \frac{\pi}{\beta(x)}
	\end{equation}
	...
	The indefinite integral $
		...
		\rightarrow\infty
	$, as $x\rightarrow L_\infty/2$, so one needs a cutoff:
	\begin{equation}
...
%	Otherwise, it will change the Rindler map, and give different entanglement entropy.
	...
	\begin{equation}
	L^2/4+a^2=L_\infty^2/4
	\end{equation}
	Where the interval for the cutoff theory is of length $L$. The entanglement entropy is
	\begin{equation}
	S=\frac{c}{6}\mop{arctanh}\frac{2x}{L_\infty}\Big|_{-L/2}^{L/2}
	=\frac{c}{3}\mop{arctanh}\frac{L}{\sqrt{L^2+4a^2}}
	=\frac{c}{3}\mop{arcsinh}\frac{L}{2a}
	\end{equation}
	with C-function
	\begin{equation}
	C(l)=\frac{c}{3}\frac{1}{\sqrt{1+\frac{4a^2}{L^2}}}
	\end{equation}
	...
	\begin{equation}
	M:D(A)\rightarrow R\times S
	\end{equation}
	\begin{equation}
	\tilde{M}:D(\tilde{A})\rightarrow R\times S
	\end{equation}
