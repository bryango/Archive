...
\newcommand{\wx}[2]{\textcolor{purple}{#1}\todo[color=teal]{\scriptsize{X: #2}}}
...
\section{Conventions}
	...
	\item Coordinate length of the boundary interval $\mcal{A}$ at the asymptotic infinity is denoted as $\tilde{l}$; the coordinate length at finite cutoff is denoted as $L$. For an interval lying on the constant time slice $t = 0$ in the Poincar\'e patch, we have $L = \Delta x$; for a boosted interval, we have $
...
\subsection{Boundary modular flow}
	...
	In such case there is a generalized Rindler transformation $f$ that maps the casual domain $\mcal{D}$ to a patch $\{(\tau,x')\}$ in some noncompact generalized Rindler spacetime, with $\pdd{\tau}$ an isometry of the spacetime. Then we have:
	\begin{equation}
		\xi = 2\pi \pdd{\tau}
		= 2\pi \sum_i s^i H_i,
	\quad
	...
	However, after the \TTbar-deformation, the scaling and special conformal transformations are no longer symmetries of the theory, so we cannot simply take $\xi$ as the modular flow in the deformed theory. 
...
\subsection{Effective local temperature and entanglement entropy}
	...
	Let's see this trick in action in CFT$_2$; in terms of the effective inverse temperature $\beta(x)$, the \textit{entropy density} is given by:
	\begin{equation}
	s = \frac{c}{6} \frac{2\pi}{\beta(x)},
	\end{equation}
	while the total entropy of the interval $\mcal{A}$ is given by:
	\begin{equation}
	S = \frac{c}{6}
		\int_\mcal{A} \frac{2\pi}{\beta(x)} \dd{x}
	\end{equation}
	For a single interval in CFT$_2$,
	\begin{equation}
		\beta(x)
		= \frac{2\pi}{\tilde{l}}
			\pqty{
	...
	\quad
		...
		= \frac{\beta(x)}{2\pi}
	\end{equation}
	Note that although $\xi$ is a Killing vector when it's extended to the bulk (as is done in later sections), it is only a \textit{conformal Killing vector} when it's restricted to the boundary. 
	...
	\quad
		...
		= \frac{\beta(x)}{2\pi},
	\end{equation}
	where we've properly rescaled the $x'$ coordinate to make the $(\tau,x')$ coordinates conformally flat. 
	
	In the case where the theory is conformally invariant, one can further drop the conformal factor $\Omega^2(\tau,x')$ from the metric, while the thermal entropy of the system remains unchanged. We now have a flat annulus:
	\begin{equation}
		\mcal{R}'
		= S^1_\beta \times [0,l'],
	\quad
...
%	, where we have: 
	...
	\end{aligned}
	\end{equation}
	Here we've used the saddle point approximation and assumed that the vacuum (the $h = \tilde{h} = 0$ states) contribution dominates. The entropy is thus given by:
	\begin{equation}
		S
		= \pqty{
				1 - \beta\, \pdd{\beta}
			} \log Z[\beta]
		= \frac{2\pi}{\beta} \frac{c}{6} l'
	\end{equation}
	With $\beta = \beta_\tau = 2\pi$, we recover the result $
		S = \frac{c}{6} l'
	$. 
	
	Note that in the above arguments we've assumed conformal symmetry, which we no longer have after a \TTbar deformation. On the other hand, as an irrelevant deformation, \TTbar is often understood as some sort of cutoff of the original CFT. With this intuition in mind, let's tweak \eqref{eq:cft_local_entropy} as follows:
	\begin{equation}
	S=\frac{c}{3}\int_{-l/2}^{l/2}
		\frac{\tilde{l}/2}{(\tilde{l}/2)^2-x^2}dx
		={c\over3} \mop{arctanh} {2x\over \tilde{l}}\bigg|^{l/2}_{-{l/2}}
	\end{equation}   
	where the integration range is given by the length $l$ of the interval $\mcal{A}$ in the \TTbar-deformed theory. If we \textit{assume} that $l$ is related to $\tilde l$ by:
	\begin{equation}
		\Big(\frac{\tilde{l}}{2}\Big)^2
		= \Big(\frac{l}{2}\Big)^2 + 2\lambda
	\label{eq:cutoff_relation}
	\end{equation}
	Then the entanglement entropy is given by
	\begin{equation}
	S=\frac{c}{6}\mop{arctanh}\frac{2x}{\sqrt{l^2 + 8\lambda}}\bigg|_{x=-l/2}^{x=l/2}
	...
	As we've demonstrated above, it seems that the local temperature trick still works for the deformed theory, assuming the mysterious relation \eqref{eq:cutoff_relation}. The meaning of this relation will be made clear in the holographic setting. As for why the trick still works in the absence of conformal symmetry, \sidenote{perhaps the reason is again, that one can deploy a Rindler-like map as in \cite{Lewkowycz:2019xse}, where the conformal factor vanishes at the endpoints, due to $2\pi$ periodicity of $\xi$, which we will show in later sections. } 
...
%	\quad
	...
	Again, we restrict to an interval centered at the origin: $
		...
		= (\frac{\tilde{l}}{2})^2
	$, where $x_\pm = -x_\mp$, and same for $t_\pm, u_\pm, v_\pm$. 
	$\tilde{l}$ is the coordinate length of the boundary interval $\mcal{A}$ at the asymptotic infinity; this corresponds to the field theory side $\tilde{l}$. 
	For now the coordinates $u_\pm, v_\pm, x_\pm, t_\pm$ and the coordinate length $\tilde{l}$ are all specified at $z\to 0$, i.e.~at the \textbf{asymptotic boundary}. Note that the RT surface lies on the plane $
		t_\pm / x_\pm = \tanh \theta
	$, namely $\theta$ is constant along the RT surface. 
...
\textbf{Summary:} We look at the flow at constant cutoff $z = z_c$, which has non-trivial $\pdd{z}$ component, which in turn leads to a violation of the $2\pi$ periodicity for $\xi$ along the horizon.
	...
	\begin{equation}
		\pqty{\frac{l}{2}}^{\!\!2}
		= \bigg(\frac{\tilde{l}}{2}\bigg)^{\!\!2} - z_c^2,
	\quad
		x_\pm^c
		= \pm \frac{L}{2}
...
\end{comment}
	...
	Which no longer has normalized $\beta$ if $z_c \ne 0$.
	 ...
	 In fact, compared to the $z_c \to 0$ situation which \textit{does} have $\beta = 1$, $\xi_c$ is rescaled by a factor of $
		\frac{L}{\tilde{l}}
		= 1/\sqrt{1 + (2z_c/L)^2}
	$, which means that:
	\begin{equation}
...
%	\hspace{2em}
%	\vspace{-2ex}
	\caption{Integral surface of the modular flow starting from $t = 0,\ z = z_c$. Here we've chosen $\frac{\tilde{l}}{2} = 1$ while $z_c = 0.4$.}
	\end{figure}
...
\pagebreak
	...
	\begin{equation}
		\dv{z}{t}
		= \frac{2tz}{\alpha z^2 + t^2 - (\tilde{l}/2)^2},
	\quad
	...
	\begin{equation}
		z = z(t;x_c,z_c)
		= \frac{
				(\tilde{l}/2)^2 + \alpha z_c^2
				- \sqrt{
					\pqty{(\tilde{l}/2)^2 - \alpha z_c^2}^2
					+ 4t^2 \alpha z_c^2
				}
			}{2\alpha z_c}
	\end{equation}
...
%	
	...
	One can check that the $(u,v,\rho)$ coordinates indeed cover the $\tilde{l}$-sized entanglement wedge specified by $\xi$. 
...
%	\vspace{-2ex}
	\caption[Integral surface of the modular flow when mapped to the planar BTZ coordinates]{%
		...
		The flow starts from $t = 0,\ z = z_c$, and here we've chosen $\frac{\tilde{l}}{2} = 1$ while $z_c = 0.1,\,0.2,\,\cdots,\,0.9$. 
	...
	\begin{equation}
		...
		= T^2 \pqty{
			1 + \frac{1}{2} \pqty{
				\frac{
					\cosh\,(2T\phi) \sqrt{
						1 - (\frac{2z_c}{\tilde{l}})^2 \sinh^2 (2T\phi)
					} - 1
				}{z_c\sinh^2 (2T\phi)}
			}^{\!\!\!\!2\ }
		},\quad
...
\pagebreak
\section{Finite temperature: entanglement entropy as gravitational charges}
...
\textbf{Summary:} We then consider constant $r$ cutoff in the Schwarzschild coordinates, which corresponds to the proposal of \cite{McGough:2016lol}. This gives a finite temperature generalization of the result of \cite{Lewkowycz:2019xse}. 
	...
	Now we consider constant cutoff at $r = r_c$. Again we need to trade the $\tilde{l}$ above with the coordinate length of the interval $L$ at the cutoff $r_c$. The relation can be solved by looking at the geodesic equation, which is slightly non-trivial this time:
	\begin{equation}
		\frac{\cosh \tilde{l} T}{\cosh LT}
	...
	\begin{equation}
		...
		= \frac{\var{T} \coth \tilde{l} T}{2G}
			\int_{-L/2}^{L/2} \dd{\phi}
			\pqty{
				1 - {\frac{r}{\sqrt{r^2 - 4T^2}}}
				\frac{\cosh 2T\phi}{\cosh T\tilde{l}}
			}
	\end{equation}
