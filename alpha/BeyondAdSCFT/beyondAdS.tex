...
\subsubsection{The worldsheet \cft{2}}
	...
	The bosonic action of the worldsheet theory is given by \wx{the familiar non-linear sigma model \needcites:}{conventions?}
	\begin{equation}
	\begin{aligned}
	...
	We can further introduce an auxiliary field $\beta$, which leads us to the \wx{Wakimoto representation \needcites:}{conventions?}
	\begin{equation}
	...
	An alternative representation is given in terms of the $\widehat{\mrm{SL}}(2,\mbb{R})_k$ WZW model. 
...
\eq{D(x,\bar x)=\int d^2z (\partial_x J \partial_x+2\partial_x^2J)(\partial_{\bar{x}} \bar{J} \partial_{\bar{x}}+2\partial_{\bar{x}}^2\bar{J}) \Phi_1}

\eq{A^a(x,\bar{x})=\int d^2z k^a(z)(\partial_{\bar{x}} \bar{J} \partial_{\bar{x}}\Phi_1+2\partial_{\bar{x}}^2\bar{J}\Phi_1)}
...
\eq{T(x)=\sum_{i=1}^p T_i(x)}

which can be also written as 
\eq{T(x)=\frac1{2k}(\partial_x J\partial_x \Phi_1+2\partial_x^2 J\Phi_1)\bar{J}(\bar{x};\bar{z})}

The OPEs are 
\eq{T(x, \bar{x}) D(y, \bar{y})\sim \frac{c_{\mathcal{M}}}{(x-y)^4}\bar{T}(\bar{y})+\cdots}
