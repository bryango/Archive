...
\section{Algebra \& Unitary Representations}
	...
	\item The structure of this algebra can be generalized to $D$ dimensions. In general, $Q$ transforms as a \mbox{(symplectic)}\footnote{
		...
		Also, higher form centeral charges are possible in higher dimensions. 
	} (pseudo)\footnote{
...
\subsection{More on conjugations}
	The bar notation in e.g.\ $\bar{Q}$ denotes \textit{some sort of} conjugation, usually Dirac or charge conjugation in 4D. More precisely, for a Dirac spinor $\psi^\bullet = (\lambda^\alpha, \bar{\chi}_{\dot{\alpha}})$, up to various constant coefficients (which may depend on the dimension $D$), we have:
	\begin{equation*}
...
\midrule
	...
	\item In components, the bar notation is actually redundant. It simply indicates that we have taken some sort of conjugation \cite{Freedman:2012zz}. To know what kind of conjugation this is, we have to read the indices. 
		...
		\begin{itemize}
		\item Dotted indices denote the complex conjugate representation. 
	...
	\item Charge conjugation is basically Dirac conjugation, but without the lowering of indices with $\mcal{C}$. We see that when $\lambda = \chi$, $\psi$ is invariant under charge conjugation: $\psi^C = \psi$. This is the reality condition, and $\psi = (\chi^\alpha,\bar{\chi}_{\dot{\alpha}})$ is a \textit{Majorana spinor}. 
	
	\item Charge conjugation is convenient as it preserves the ordering of matrices (since it does not include a transpose). On the other hand it works just like Dirac conjugation, switching between left and right-handed components. 
	
	\item When the indices are suppressed, they are assumed to be contracted with $\mcal{C}$. In this case we do need to specify the meaning of $\bar{\psi}$. 
	
	When the indices are suppressed, charge and Dirac conjugations are equivalent (up to some constant coefficients) as they differ only by index positions. Similarly, complex and Majorana conjugations are equivalent. 
	
	For majorana spinors (such as the supercharges $Q$), we have $\lambda = \chi$, and all four conjugations are equivalent when the indices are suppressed. 
	
	\item In 4D we usually take $\bar{\psi}$ to denote Dirac conjugate, as in \S2 of \textit{Supergravity} \cite{Freedman:2012zz}. However, since \S3 they switch their conventions and $\bar{\psi}$ denotes Majorana conjugate instead\footnote{
		See \S3.2 of \textcite{Freedman:2012zz}. 
	}. \textcite{Polchinski:1998rr} seems to define $\bar{\psi}$ as Dirac conjugate\footnote{
	...
	\newparagraph
	These have always been confusing concepts for me. For a pedagogical treatment in general dimensions, see \S2 and \S3 of \textit{Supergravity} \cite{Freedman:2012zz}. One can then compare it to the results in 4D, discussed in Appendix A.4 to A.8 of \textcite{Figueroa-OFarrill:2001xbd}. Fortunately their conventions seem to agree with each other, most of the time. 
